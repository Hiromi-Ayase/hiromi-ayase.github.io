		...
		img.onload = function() {
			...
			for (var m = 1; PIECE_SIZE + m * PIECE_DISTANCE < width; m ++) {
				for (var n = 1; PIECE_SIZE + n * PIECE_DISTANCE < height; n ++) {
					...
					var x = Math.floor(Math.random() * width) - m * PIECE_DISTANCE + PIECE_SIZE;
					var y = Math.floor(Math.random() * height) - n * PIECE_DISTANCE + PIECE_SIZE;
	...
	function setCluster(cluster) {
		...
		$("#" + id).draggable({
			...
			stop : function(e) {
				...
				var cX = cluster.centerX + cluster.x;
				var cY = cluster.centerY + cluster.y;
				var autoMove = false;
				if (cX <= 0) {
					image.css("left", (-cluster.centerX + 10) + "px");
					autoMove = true;
				}
				if (cX >= width * PUZZLE_SIZE) {
					image.css("left", (width * PUZZLE_SIZE - cluster.centerX - 10) + "px");
					autoMove = true;
				}
				if (cY <= 0) {
					image.css("top", (-cluster.centerY + 10) + "px");
					autoMove = true;
				}
				if (cY >= height * PUZZLE_SIZE) {
					image.css("top", (height * PUZZLE_SIZE - cluster.centerY - 10) + "px");
					autoMove = true;
				}
				cluster.move(image.context.offsetLeft, image.context.offsetTop);
...
function pieceCluster(pieceList, x, y, angle, image) {
	...
	this.join = function(cluster) {
		...
		var newCluster = new pieceCluster(list, this.x, this.y, this.angle,
				this.image);
		return newCluster;
	};
...
/**
 * Edit imageData to a piece
 * 
 * @param imageData
 */
function makePiece2(imageData) {
	var imageHeight = imageData.height;
	var imageWidth = imageData.width;
	var centerY = imageHeight / 2;
	var centerX = imageWidth / 2;

	var maxLen = imageHeight < imageWidth ? imageHeight / 2 : imageWidth / 2;

	for ( var y = 0; y < imageHeight; y++) {
		for ( var x = 0; x < imageWidth; x++) {
			var i = (y * imageWidth + x) * 4 + 3;
			var len = getDistance(centerX, centerY, x, y);
			var lengthRate = 1 - len / maxLen;
			lengthRate = lengthRate < FILL_SIZE ? lengthRate / FILL_SIZE : 1;
			imageData.data[i] = lengthRate < 0 ? 0 : lengthRate * 256;
		}
	}
}
