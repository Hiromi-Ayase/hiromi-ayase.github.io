...
module ShizimilyRogue.View {
    ...
    class View extends enchant.Group {
        ...
        private moveCamera(): void {
            var x = this.objects[Common.PLAYER_ID].x;
            var y = this.objects[Common.PLAYER_ID].y;
            this.x = -x + (VIEW_WIDTH - OBJECT_WIDTH) / 2;
            this.y = -y + (VIEW_HEIGHT - OBJECT_HEIGHT) / 2;
        }
        ...
        private static getPlayerInstance(obj: Common.IUnit): ViewObject {
            ...
            var actionAnimation = (sprite: enchant.Sprite, action: Common.Action, speed: number) => {
                if (action.isAttack()) {
                    ...
                    sprite.tl.delay(5).then(() => { sprite.frame = x[obj.dir][1] }).delay(5);
